...
<div class="relative min-h-screen overflow-hidden bg-black text-white">
	{#if loading}
		...
		<section class="relative h-screen overflow-hidden">
			...
			<div
				class="absolute inset-0 bg-linear-to-r from-black/70 via-black/20 to-transparent"
			></div>
			...
			<div class="absolute inset-0 z-10 flex items-end px-8 pb-36 md:px-16">
				<div class="max-w-2xl">
					<div class="h-16 w-72 animate-pulse rounded-2xl bg-zinc-800"></div>

					<div class="mt-4 flex gap-2">
						<div class="h-6 w-20 animate-pulse rounded-full bg-zinc-800"></div>
						<div class="h-6 w-24 animate-pulse rounded-full bg-zinc-800"></div>
						<div class="h-6 w-16 animate-pulse rounded-full bg-zinc-800"></div>
					</div>

					<div class="mt-4 h-4 w-40 animate-pulse rounded bg-zinc-800"></div>

					<div class="mt-4 space-y-2">
						<div class="h-4 w-full animate-pulse rounded bg-zinc-800"></div>
						<div class="h-4 w-11/12 animate-pulse rounded bg-zinc-800"></div>
						<div class="h-4 w-9/12 animate-pulse rounded bg-zinc-800"></div>
					</div>

					<div class="mt-8 h-14 w-44 animate-pulse rounded-xl bg-zinc-800"></div>
				</div>
			</div>
		</section>

		<!-- ROW SKELETONS -->
		<div class="space-y-12 px-6 pb-12">
			{#each Array.from({ length: 4 }, (_, row) => row) as row (row)}
				<div>
					<div class="mb-5 h-8 w-52 animate-pulse rounded bg-zinc-800"></div>

					<div class="flex gap-4 overflow-hidden">
						{#each Array.from({ length: 6 }, (_, i) => i) as i (i)}
							<div
								class="aspect-2/3 w-40 shrink-0 animate-pulse rounded-2xl bg-zinc-900"
							></div>
						{/each}
					</div>
				</div>
			...
			<section class="relative z-10 h-screen overflow-hidden">
				{#each featuredMovies as movie, i (movie.id)}
					...
					>
						...
						<div
							class="absolute inset-x-0 top-0 h-32 bg-linear-to-b from-black/50 to-transparent"
						></div>
					</div>
				{/each}
